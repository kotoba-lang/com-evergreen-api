(ns evergreen-api.main
  "Kotodama WASM entrypoint for the Evergreen_api clean-room actor (L4) — Clojure port.
  ...
  facts (`evergreen_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "evergreen_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "evergree_tra"
    :fields [:gtin :description :brand]        :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"      :plural "events"      :id-prefix "evergree_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"   :plural "documents"   :id-prefix "evergree_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container"  :plural "containers"  :id-prefix "evergree_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"      :plural "parties"     :id-prefix "evergree_par"
    :fields [:gln :name :role]                 :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "evergree_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"evergreen_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "evergreen_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :evergreen_api-compat/ready)
